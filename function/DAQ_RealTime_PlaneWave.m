function  [NumsPerFile,File_nums,frameN] = DAQ_RealTime_PlaneWave(AcqConfig,ScanList,prf,lasting_time,frame_rate,filepath,is_save,is_show,hfig)
...
JsAper.Jswin       = single(AcqConfig.Rx.aperture.js_win');
JsAper.Js_Fn_depth = single(AcqConfig.Rx.aperture.fn_depth);
if(isscalar(AcqConfig.Rx.aperture.fn_value))
    AcqConfig.Rx.aperture.fn_value(1,2)  =AcqConfig.Rx.aperture.fn_value(1,1);
end
JsAper.Js_Min_Aper = single(AcqConfig.Rx.aperture.min_Aper);
JsAper.Js_Max_Aper = single(AcqConfig.Rx.aperture.max_Aper);
JsAper.Js_Max_Fn   = single(max(AcqConfig.Rx.aperture.fn_value));
JsAper.Js_Min_Fn   = single(min(AcqConfig.Rx.aperture.fn_value));
JsAper.Js_Fn_value = single(AcqConfig.Rx.aperture.fn_value);
...
single_JsWin = single(JsAper.Jswin);
...
gpu_handle = calllib('US_GPU', 'initializeBeamformingGPU', ...
    ...
    AcqConfig.Probe.element_radius, ...
    AcqConfig.Rx.Revpt,...
    Nz,...
    Nx, ...
    AcqConfig.Tx.sos, ...
...
% 存bf结果
all_bf_data = zeros(Nz, Nx, length(ScanList));
...
while(frameid <= max_frameid)
    ...
    if is_show==1
        ...
        for angle_i = 1:length(ScanList)
            ...
            img_temp = reshape(bfdata_ptr.Value, [Nz Nx]);
