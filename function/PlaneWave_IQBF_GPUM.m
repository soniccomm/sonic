function PlaneWave_IQBF_GPUM(AcqConfig,postpara,datapath,filepath,savepara,is_show)


chmap = AcqConfig.Probe.rx_ele_map;
fs = AcqConfig.Rx.fs;
c = AcqConfig.Rx.sos;
RxChannel = AcqConfig.Rx.channel;
...
% 计算发射和接收延时
steer = deg2rad(AcqConfig.Rx.Steering); % 角度制
...
single_Steering = single(AcqConfig.Rx.Steering);
...
% 5. 采集设置
% 原始数据长度
BData_len = numel(steer) *(2 * Rx_sample_num*RxChannel+32) ;
